...
public class GameController implements ActionListener {
	...
	public static void startNextScene(String sceneName,GameState gameState)
	{
		...
		//TODO Write code for starting next scene.
	...
	public static void startNextScreen(String screenName,GameState gameState)
	{
		...
		for(Screen screen : screenList)
		{
			...
			{
				if(gameElement.getGameElementIdentifier().getId().trim().equals(screenName.trim()))
				{
					resultScreen = screen;
					flag = true;
					break;
	...
	public static boolean gameElementToStart(GameState gameState) {
		...
		if (currentGameElement < screenToStart.getGameElements().size()) {
			System.out.println("GameElement id is... >>> " + screenToStart.getGameElements().get(0).getGameElementIdentifier().getId());
	...
	/*public static boolean propToStart(GameState gameState) {
		boolean isPropStart = false;
		if (currentProp < gameElementToStart.getGameElements().size()) {
			isGameElementStart = true;
	...
	public static void propToEnd(GameState gameState) {
		currentGameElement++;
		try {
			gameElementToStart.gameElementEnd(gameState);
		} catch (Exception e) {
	...
	public static void viewListener(Observable observable, GameState gameState) {
		...
		if (Screen.class.isInstance(observable)) {
			...
			if (message == Message.EndComplete) {
				...
				System.out.println(isScreenStart());
		...
		if (GameElement.class.isInstance(observable)) {
			...
			if (message == Message.EndComplete) {
				...
				// To check whether there are any new gameElements to start.
